 ...
 * @access public
 */
class Shopire_Customizer_Range_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script(
			'shopire-controls',
			SHOPIRE_THEME_INC_URI . '/customizer/controls/js/controls.js',
			array(
				'jquery',
				'customize-base',
				'jquery-ui-button',
		...
		wp_enqueue_style( 'shopire-controls', SHOPIRE_THEME_INC_URI . '/customizer/controls/css/controls.css' );
		wp_enqueue_style( 'font-awesome', SHOPIRE_THEME_URI . '/assets/vendors/css/all.min.css' );
	}
	
	/**
	 * Shopire_Customizer_Range_Control constructor.
	 *
 ...
 */
function shopire_sanitize_range_value( $input ) {
	if ( shopire_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function shopire_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
